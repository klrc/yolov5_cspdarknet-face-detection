...
from model_loss import create_optimizer, create_scheduler, EarlyStop, YOLOv5FaceQualityLoss
...
def train(
    ...
    train_loader = InfiniteDataLoader(
        ...
        training=True,
        batch_size=batch_size,
        collate_fn=RelativeHumanDataset.collate_fn,
    ...
    # load anchors
    anchors_preset_tensor = torch.clone(model.head.anchors)
    strides_tensor = torch.tensor(strides)  # strides computed during build
    auto_anchors = check_anchors(train_loader.dataset, anchors_preset_tensor.cpu(), strides_tensor.cpu(), 4.0)
    model.head.anchors = auto_anchors.to(device)
    ...
    # create loss
    loss_func = YOLOv5FaceQualityLoss(model)
    ...
    if wandb_enabled:
        wandb.init(project=project_name, dir=save_dir, name=model_name, mode="offline")
        wandb.watch(model)
        wandb.run.log_code("./", include_fn=lambda path: path.endswith(".py"))
        wandb.config.update(raw_args)
        logger.success("Wandb logger created")
    logger.success(f"Model name: {model_name}")
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="YOLOv8 Face Detection Training Configuration")

    # 添加命令行参数
    parser.add_argument("--project_name", type=str, default="face_detection", help="项目名称")
    ...
    parser.add_argument("--loss_titles", nargs="+", default=["box", "lmk", "sta", "qty", "age", "gdr", "ado"], help="默认yolov8损失函数项")
    parser.add_argument("--device", type=str, default="mps", help="设备类型: cuda, mps, cpu, 0, 1, ...(显卡device id)")
    parser.add_argument("--trainset_path", type=str, default="/Volumes/ASM236X/datasets/relative_human_mixed_0124/images/train", help="训练集images文件夹位置, 会根据images->labels规则寻找标签文件夹")
    parser.add_argument("--valset_path", type=str, default="/Volumes/ASM236X/datasets/relative_human_mixed_0124/images/val", help="测试集文件夹位置")
    ...
    parser.add_argument("--class_names", nargs="+", default=RelativeHumanDataset.class_names, help="类别名称")
    ...
    train(**vars(args), raw_args=vars(args))
