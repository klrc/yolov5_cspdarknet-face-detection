...
from box import rectangular_training, letterbox, xy2xyn, xyn2xy, xywhn2xyxy, xyxy2xywhn
from transforms import AlbumentationsPreset, augment_hsv, fake_osd, mosaic4, random_perspective, random_smear_xyxy, flip_landmarks_and_occlusions_lr
from data_table import DATASET_NUM_DIMS, IDX_LANDMARKS


class RelativeHumanDataset(Dataset):
    ...
    def __init__(
        ...
        self.albumentations = AlbumentationsPreset(
            random_illumination=random_illumination,
    ...
    def __getitem__(self, index):
        ...
        if self.training and random.random() < self.mosaic:
            ...
            if labels.size:  # normalized xywh to pixel xyxy format
                ...
                for il in range(IDX_LANDMARKS, IDX_LANDMARKS + 10, 2):
                    labels[:, il : il + 2] = xyn2xy(labels[:, il : il + 2], ratio[0] * w, ratio[1] * h, padw=pad[0], padh=pad[1])
            border = (0, 0)
        ...
        if self.training:
            ...
            # Random smear
            image, labels = random_smear_xyxy(image, labels, self.random_smear)

            nl = len(labels)  # number of labels
            if nl:
                labels[:, 1:5] = xyxy2xywhn(labels[:, 1:5], w=image.shape[1], h=image.shape[0], clip=True, eps=1e-3)
                for il in range(IDX_LANDMARKS, IDX_LANDMARKS + 10, 2):
                    labels[:, il : il + 2] = xy2xyn(labels[:, il : il + 2], w=image.shape[1], h=image.shape[0], clip=True, eps=1e-3)
            ...
            if random.random() < self.fliplr:
                ...
                if nl:
                    labels[:, 1] = 1 - labels[:, 1]
                    labels = flip_landmarks_and_occlusions_lr(labels)

            # Random fake OSD
            if random.random() < self.fake_osd:
                image = fake_osd(image)
        ...
        # Transform landmarks to relative labels
        if len(labels) > 0:
            w = labels[:, 3]
            h = labels[:, 4]
            x1 = labels[:, 1] - 0.5 * w
            y1 = labels[:, 2] - 0.5 * h
            for il in range(IDX_LANDMARKS, IDX_LANDMARKS + 10, 2):
                labels[:, il] = ((labels[:, il] - x1) / w).clip(0, 1)  # lmk x
                labels[:, il + 1] = ((labels[:, il + 1] - y1) / h).clip(0, 1)  # lmk y

        nl = len(labels)  # number of labels
        labels_out = torch.zeros((nl, DATASET_NUM_DIMS + 1))
        if nl:
