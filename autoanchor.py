...
def check_anchors(dataset, anchors, strides, thr=4.0):
    ...
    if bpr > 0.98:  # threshold to recompute
        ...
        if new_bpr > bpr:  # replace anchors
            new_anchors = torch.tensor(new_anchors, device=anchors.device).type_as(anchors).view_as(anchors)
            check_anchor_order(new_anchors, strides)  # must be in pixel-space (not grid-space)
