...
from model import ConvModule, yolov8n_facecap_v2
...
def export(weight: str, device, input_shape, input_names, output_names, opset_version, enable_onnxsim):
    ...
    onnx_path = f"{model_dir}/{model_name}_{input_shape[-1]}x{input_shape[-2]}.onnx"
    anchors_path = f"{model_dir}/{model_name}.anchors"
    ...
    with open(anchors_path, "w") as f:
        json.dump(model.head.anchors_for_porting, f)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="YOLOv8 Face Detection Export Configuration")
    ...
    parser.add_argument("--output_names", nargs="+", default=["head_0", "head_1", "head_2"], help="输出张量名称")
